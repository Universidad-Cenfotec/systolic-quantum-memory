...
class CMaxValidatorNot:
   
    _ONE_QUBIT_GATE_CANDIDATES = ["x", "sx", "rz"]

    # -- Constructor ----------------------------------------------------------

    def __init__(self, N: int = 1, backend=None, mitigation_config: dict | None = None) -> None:
        ...
        if backend is not None:
            ...
            print(f"[CMaxValidatorNot] Using IBM hardware backend: {self.backend.name}")
        else:
        ...
        # 3. Detect native 1Q gate and extract average error.
        self.native_1q_gate: str = ""   # filled by _extract_avg_1q_error
        self.x_error: float     = self._extract_avg_1q_error()

        # 4. Theoretical NOT pair error (2*N native 1Q gates in series, i.i.d.)
        #    Kept as reference to compare against empirical r from RB.
        self.p_not_teorico: float = 1.0 - (1.0 - self.x_error) ** (2 * N)
        ...
        if self.zne_enabled or self.rem_enabled:
            print(f"[CMaxValidatorNot] Mitigation: ZNE={'ON' if self.zne_enabled else 'OFF'}, "
                  f"REM={'ON' if self.rem_enabled else 'OFF'}")

    # -- Extraction of calibration parameters ----------------------------------

    def _extract_avg_1q_error(self) -> float:
        ...
        for candidate in self._ONE_QUBIT_GATE_CANDIDATES:
            if candidate in gate_errors:
                self.native_1q_gate = candidate
                return float(np.mean(gate_errors[candidate]))

        raise RuntimeError(
            f"No known 1Q gate found among "
            f"{self._ONE_QUBIT_GATE_CANDIDATES} in '{self.backend.name}'. "
    ...
    def _save_rb_results_to_csv(
        ...
        with open(csv_path, 'w', newline='') as f:
            ...
            writer.writerow(["SQM RB Characterization Results (NOT protocol)"])
            ...
            writer.writerow(["p_not_theory", f"{self.p_not_teorico:.6f}"])
            writer.writerow([])
            
            # Write data columns
            writer.writerow(["m (NOT cycles)", "F_emp (fidelity)", "F_fit (fitted)"])
    ...
    def print_rb_results(self, popt: np.ndarray) -> float:
        ...
        print(f"  {'p':<12}  {self.p_fit:>12.6f}  Process decay per NOT pair")
        ...
        print(f"    p_not_theory ({self.native_1q_gate.upper()})      = "
              f"{self.p_not_teorico:.6f}  ({self.p_not_teorico * 100:.4f} %)")
        print(f"    r_empirical (RB)               = "
              f"{self.r_empirico:.6f}  ({self.r_empirico * 100:.4f} %)")

        diff_abs = abs(self.r_empirico - self.p_not_teorico)
        diff_rel = (diff_abs / self.p_not_teorico * 100) if self.p_not_teorico > 0 else float("inf")
    ...
    def _plot_rb_curve(
        ...
        ax.set_xlabel("m  (number of NOT pairs)", fontsize=12)
        ax.set_ylabel("F(m)  - base state survival", fontsize=12)
        ax.set_title(f"NOT Pair Decay Curve (N={self.N}, d={self.d})", 
    ...
    def theoretical_fidelity(self, n_cycles: int) -> float:
      
        if n_cycles < 0:
            raise ValueError(f"n_cycles must be >= 0, received: {n_cycles}")
        return self.A_fit * self.p_fit ** n_cycles + self.B_fit

    # -- Extrapolation validation (n vs 2n) -------------------------------------

    def run_extrapolation_test(self, n: int = 10) -> None:
        
        gate_label = self.native_1q_gate.upper()
        print("=" * 65)
        print(f"  SQM -- Phase B.4: Extrapolation Validation (n={n})")
        print("=" * 65)
        print(f"\n  p_{gate_label.lower()} = {self.x_error:.6f}  |  "
...
if __name__ == "__main__":
    ...
    print(f"\n[FINAL RESULT]  C_MAX = {c_max} NOT pairs  "
          f"(r_emp = {r_emp:.4f},  p_not_theory = {validator.p_not_teorico:.4f})")
    ...
    print(f"    r_emp={r_emp:.4f}  vs  p_not_theory={validator.p_not_teorico:.4f}")
