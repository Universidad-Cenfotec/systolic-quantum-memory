...
class AerSimulatorBackend(BackendInterface):
    ...
    use_native_delay : bool
        Always False for AerSimulator (uses id() gates for idle periods)
    """
    ...
    def __init__(
        ...
        t1_ns : float, optional
            T1 relaxation time in nanoseconds (default: 192566 ns = 192.566 us)
        t2_ns : float, optional
            T2 dephasing time in nanoseconds (default: 35887 ns = 35.887 us)
    ...
    def use_native_delay(self) -> bool:
        """AerSimulator always uses id() gates, not native delay()."""
        return False
